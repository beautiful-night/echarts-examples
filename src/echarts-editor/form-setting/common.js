...
export const createHierarchy = (name, controls) => {
  return {
    name: name,
    type: 'combo',
    label: '',
 ...
 * @param label
 */
export const color = (name, label, defaultColor, labelRemark) => {
  return {
    type: 'color',
    format: 'rgba',
    pipeIn: value => {
 ...
 * @param label
 */
export const trueSwitch = (name, label, labelRemark) => {
  return {
    type: 'switch',
    name: name,
    mode: 'inline',
 ...
 * @param label
 */
export const falseSwitch = (name, label, labelRemark) => {
  return {
    type: 'switch',
    name: name,
    mode: 'inline',
 ...
 * @param options
 */
export const select = (name, label, options, labelRemark) => {
  return {
    type: 'select',
    name: name,
    label: label,
 ...
 * @param collapsed
 */
export const fieldSet = (label, controls, collapsed = false) => {
  return {
    type: 'fieldSet',
    title: label,
    collapsable: true,
 ...
 * @param controls
 */
export const visibleOn = (visibleOn, controls) => {
  return {
    type: 'container',
    visibleOn: visibleOn,
    body: controls,
 ...
 */
export const number = (name, label, labelRemark, defaultNumber, min = -1e4, max = 1e4, step = 1) => {
  const control = {
    type: 'input-number',
    name: name,
    label: label,
 ...
 * @param defaultText
 */
export const text = (name, label, labelRemark, defaultText) => {
  const control = {
    type: 'input-text',
    name: name,
    label: label,
 ...
 */
export const viewportControl = (name, label, labelForRadio, keywordList) => {
  return [
    {
      type: 'button-group',
      name: name,
 ...
 * @param label 组件名
 */
export const viewport = (scope, label) => {
  return {
    type: 'fieldSet',
    title: '离容器边距',
    collapsable: true,
    ...
    mountOnEnter: true,
    body: [
      ...viewportControl(`${scope}left`, `${label}离容器左侧的距离`, '左侧距离值类型', ['auto', 'left', 'center', 'right']),
      ...viewportControl(`${scope}top`, `${label}离容器上侧的距离`, '上侧距离值类型', ['auto', 'top', 'middle', 'bottom']),
 ...
 * @param label
 */
export const selectedMode = label => {
 ...
 * @param scope
 */
export const animation = (scope, collapsed = true) => {
  let prefix = ''
  if (scope) {
    prefix = `${scope}.`
  }
  ...
  return fieldSet('动画', [
    ...
    visibleOn(`this.${prefix}animation`, [
      number(`${prefix}animationThreshold`, '是否开启动画的阈值', '当单个系列显示的图形数量大于这个阈值时会关闭动画', 2000),
      number(`${prefix}animationDuration`, '初始动画的时长', '支持回调函数，可以通过每个数据返回不同的时长实现更戏剧的初始动画效果，不过只能写代码', 1000),
      select(`${prefix}animationEasing`, '初始动画的缓动效果', easing),
      number(`${prefix}animationDelay`, '初始动画的延迟', '初始动画的延迟，支持回调函数，可以通过每个数据返回不同的 delay 时间实现更戏剧的初始动画效果', 0),
    ]),
    number(`${prefix}animationDurationUpdate`, '数据更新动画的时长', '支持回调函数，可以通过每个数据返回不同的时长实现更戏剧的更新动画效果', 200),
    select(`${prefix}animationEasingUpdate`, '数据更新动画的缓动效果', easing),
    number(`${prefix}animationDelayUpdate`, '数据更新动画的延迟', '支持回调函数，可以通过每个数据返回不同的 delay 时间实现更戏剧的更新动画效果'),
  ])
}
 ...
 */
export const numberOrArray = (name, label, labelForSwitch, defaultNumber = 5, defaultArray = [0, 0, 0, 0]) => {
  return [
    {
      type: 'group',
      body: [
 ...
 * @param defaultArray 默认数组
 */
export const vector = (name, label, labelForSwitch, labelRemark, defaultNumber = 0, defaultArray = [0, 0, 0, 0]) => {
  return [
    {
      type: 'input-array',
      name: name,
 ...
 * 阴影相关的控件
 */
export const shadowControls = scope => {
  return {
    type: 'fieldSet',
    title: '阴影',
    collapsable: true,
 ...
 * @param keywordList
 */
export const keywordOrNumber = (name, label, labelForSwitch, keywordList, defaultNumber = 100) => {
  return {
    type: 'group',
    body: [
      {
 ...
 * @param remark
 */
export const keywordOrString = (name, label, labelForSwitch, keywordList, defaultString = '', remark) => {
  return {
    type: 'group',
    body: [
      {
      ...
      },
      {
        ...
        name: name,
        label: label,
        visibleOn: `typeof(this.${name}) === 'undefined' || ${JSON.stringify(keywordList)}.indexOf(this.${name}) !== -1`,
        options: keywordList,
      },
    ],
  }
 ...
 * @param controls
 */
export const objectOrArray = (name, labelForSwitch, controls) => {
  return [
    {
      type: 'switch',
      label: labelForSwitch,
 ...
 * @param label
 */
export const commonStyle = (scope, label) => {
  return [
 ...
 * @param keywordList
 */
export const booleanOrKeyword = (name, label, keywordList, defaultBoolean = true) => {
  return {
    type: 'select',
    name: name,
    label: label,
 ...
 * @param keywordList
 */
export const enumControl = (name, label, keywordList, defaultValue, labelRemark) => {
  return {
    type: 'select',
    name: name,
    label: label,
 ...
 * @param keywordList
 */
export const numberOrPercentage = (name, label, labelForSwitch, labelRemark, defaultPercent = '1%', defaultNumber = 100) => {
  return [
    {
      type: 'group',
      body: [
 ...
 * @param scope
 */
export const textStyleControls = (scope, label) => {
  return createHierarchy(scope, [
    {
      ...
      body: [
        ...
        keywordOrNumber('fontWeight', `${label}文字字体的粗细`, '字体粗细格式使用数字', ['normal', 'bold', 'bolder', 'lighter']),
        {
          type: 'input-text',
          name: 'fontFamily',
          label: `${label}文字的字体`,
 ...
 * @param option
 */
const buildOneOption = (scope, name, option) => {
  if (name.indexOf('<') !== -1) {
    return
  }
  if (name.indexOf('.0') != -1) {
  ...
  if (uiControlType === 'boolean') {
    ...
    if (uiControl.options) {
      return enumControl(
        name,
        label,
        uiControl.options.split(',').map(item => item.trim()),
        uiControl.default,
        remark
      )
    } else {
...
export const buildGroupOptions = (scope, parentName, options) => {
  let controls = []
 ...
 * @param options
 */
export const buildOptions = (scope, options) => {
  const commonStyleKeys = new Set(['backgroundColor', 'borderColor', 'borderWidth', 'borderRadius'])
  ...
  for (const name in options) {
    ...
    if (name.startsWith('textStyle') || name.startsWith('subtextStyle') || name.startsWith('pageTextStyle') || name.startsWith('animation')) {
      continue
    }
    // TODO: 后续单独处理
